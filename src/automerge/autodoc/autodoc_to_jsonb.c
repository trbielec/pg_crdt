...
static JsonbValue *_am_walk_map(autodoc_Autodoc *doc, AMobjId const *objid,
                                JsonbParseState *state);

static JsonbValue *_am_walk_list(autodoc_Autodoc *doc, AMobjId const *objid,
                                 JsonbParseState *state);
...
static JsonbValue *_am_walk_map(autodoc_Autodoc *doc, AMobjId const *objid,
  ...
  while ((itemkey = AMitemsNext(&keys, 1)) != NULL) {
    ...
    item = AMstackItem(&doc->stack, AMmapGet(doc->doc, objid, bs, NULL),
                       _abort_cb, NULL);
    ...
    case AM_VAL_TYPE_F64:
      if (AMitemToF64(item, &floatv)) {
        valdatum =
            DirectFunctionCall1(float8_numeric, Float8GetDatumFast(floatv));
        val.type = jbvNumeric;
        val.val.numeric = DatumGetNumeric(valdatum);
        pushJsonbValue(&state, WJB_VALUE, &val);
      } else {
...
static JsonbValue *_am_walk_list(autodoc_Autodoc *doc, AMobjId const *objid,
  ...
  while ((itemkey = AMitemsNext(&items, 1)) != NULL) {
    ...
    item = AMstackItem(&doc->stack, AMlistGet(doc->doc, objid, itempos, NULL),
                       _abort_cb, NULL);
