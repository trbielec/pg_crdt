...
PG_FUNCTION_INFO_V1(autodoc_create_mark_bool);
Datum autodoc_create_mark_bool(PG_FUNCTION_ARGS) {
  autodoc_Autodoc *doc;
  text *path, *name;
  AMitem *item, *valitem;
  size_t start, end;
  ...
  valitem = AMstackItem(&doc->stack, AMitemFromBool(val), _abort_cb,
