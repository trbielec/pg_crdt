...
autodoc_Autodoc *_autodoc_traverse_put_text(autodoc_Autodoc *doc,
                                            ...
                                            const char *expr, const char *val,
                                            bool insert);
  ...
  while (*p) {
    if (*p == '.') { // Start of a map key
      ...
      if (valtype == AM_VAL_TYPE_OBJ_TYPE) {
        return _autodoc_traverse_put_text(doc, AMitemObjId(item), p, val,
                                          insert);
      } else {
        item = AMstackItem(&doc->stack,
                           AMmapPutObject(doc->doc, AM_ROOT,
                                          AMstr(token.value.key),
                                          AM_OBJ_TYPE_TEXT),
                           _abort_cb, NULL);
        AMstackItem(&doc->stack,
                    AMspliceText(doc->doc, AMitemObjId(item), 0, 0, AMstr(val)),
      ...
      if (valtype == AM_VAL_TYPE_OBJ_TYPE) {
        ...
        item =
            AMstackItem(&doc->stack,
                        AMlistPutObject(doc->doc, container, token.value.index,
                                        insert, AM_OBJ_TYPE_TEXT),
